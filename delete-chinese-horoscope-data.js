// delete-chinese-horoscope-data.js - Enhanced with safety features
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    switch (arg) {
      ...
      case '--daily-only':
        ...
        break;
      case '--weekly-only':
        config.types = ['weeklyChineseHoroscope'];
        break;
      case '--yearly-only':
        config.types = ['yearlyChineseHoroscope'];
        break;
      case '--sign':
        if (i + 1 < args.length) {
...
function showHelp() {
  console.log(`
Chinese Horoscope Delete Script

Usage: node delete-chinese-horoscope-data.js [OPTIONS]
...
Examples:
  node delete-chinese-horoscope-data.js --dry-run
  node delete-chinese-horoscope-data.js --daily-only --sign rat
  node delete-chinese-horoscope-data.js --date 2025-09-09
  node delete-chinese-horoscope-data.js --weekly-only --force
  `);
}
...
async function buildQuery(docType, config) {
  ...
  // Add sign filter
  if (config.signs.length < chineseZodiacSigns.length) {
...
async function deleteHoroscopeData() {
  ...
  console.log(`[${dayjs().format('YYYY-MM-DD HH:mm:ss')}] ${config.dryRun ? 'DRY RUN - ' : ''}Starting horoscope deletion...`);
  ...
  try {
    ...
    if (!config.force) {
      const confirmed = await askConfirmation(`Are you sure you want to DELETE ${totalToDelete} horoscope documents? This action cannot be undone.`);
      if (!confirmed) {
