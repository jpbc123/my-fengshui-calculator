      ...
      try {
        const [articles, kuaData] = await Promise.all([
          client.fetch<SanityArticle[]>(
            `*[_type == "article" && ("fengshui" in tags || "kua" in tags || "ba zhai" in tags)] | order(publishDate desc)[0...3]{title, "slug": slug.current}`
          ),
              ...
              {loading ? (
                ...
                <ul className="space-y-2 text-sm">
                  {relatedArticles.map((article, index) => (
                    <li key={index}>
                      <Link
                        to={`/articles/${article.slug}`} // <-- Changed this line
