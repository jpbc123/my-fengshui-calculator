...
export default function WesternDailyHoroscope() {
  ...
  const fetchHoroscope = useCallback(async (sign: string, period: PeriodTabType, category: CategoryTabType) => {
    setLoading(true);
    setError(null);
  ...
  // Effect to fetch horoscope when sign, period, or category changes
  useEffect(() => {
    if (selectedSign) {
      fetchHoroscope(selectedSign, selectedPeriod, selectedCategory);
    }
  }, [selectedSign, selectedPeriod, selectedCategory, fetchHoroscope]);
  ...
  const renderHoroscopeText = () => {
    if (loading) return <p className="text-white/70">Loading horoscope...</p>;
    if (error) return <p className="text-red-400">{error}</p>;
    if (!selectedSign) return <p className="text-white/70">Spin the wheel to reveal your horoscope.</p>;
    if (!horoscopeContent) return <p className="text-white/70">Horoscope not available.</p>;
                ...
                {(['today', 'yesterday', 'weekly', 'yearly'] as PeriodTabType[]).map((period) => (
                  <button
                    ...
                    className={`px-3 py-1 rounded-md text-sm mb-2 ${
                      selectedPeriod === period ? "bg-gold text-black font-bold" : "bg-white/10 text-black/70 hover:bg-white/20"
                    }`}
                  >
              ...
              <div className="flex flex-wrap space-x-2 mb-4">
                {(['overview', 'love', 'career', 'wealth', 'social', 'lucky_color', 'lucky_number'] as CategoryTabType[]).map((category) => (
                  <button
                    ...
                    className={`px-3 py-1 rounded-md text-sm mb-2 ${
                      selectedCategory === category ? "bg-gold text-black font-bold" : "bg-white/10 text-black/70 hover:bg-white/20"
                    }`}
                  >
