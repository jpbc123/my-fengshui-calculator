...
// Make sure to replace with your actual Supabase URL and Anon Key from your .env file
...
export default function ContactUs() {
  ...
  const handleChange = (
    ...
    if (name === "confirmEmail" || name === "email") {
      ...
      } else if (name === "email" && value !== formData.confirmEmail) {
        setEmailMatchError("Emails do not match.");
      } else {
  ...
  const handleSubmit = async (e) => {
    ...
    const { error } = await supabase.from("contact_messages").insert([dataToSubmit]);
