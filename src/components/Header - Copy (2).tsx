...
const megaMenuConfig = {
  ...
  games: {
    title: "Games & Fun",
    items: [
      { name: "Lucky Numbers Generator", href: "/lucky-numbers", description: "Generate a set of lucky numbers for any occasion." },
      { name: "Name Compatibility", href: "/name-compatibility", description: "See if your names are a perfect match." },
      { name: "Fortune Cookie", href: "/cookie", description: "Open a digital fortune cookie for a glimpse into your future." },
      { name: "Chinese Zodiac Compatibility", href: "/chinese-compatibility", description: "Find out if you and your friends' signs align." },
      { name: "Western Zodiac Compatibility", href: "/western-compatibility", description: "Discover the astrological bond between you and your friends." },
    ],
  },
  features: {
    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-gradient-to-r from-purple-900 via-indigo-900 to-gray-400 via-50% border-b border-purple-400/30 shadow-lg">
      <div className="container mx-auto px-4 h-16 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-6 text-white font-medium relative z-50">
		  ...
		  {/* 2. Games Mega Menu - Now second in the order */}
          <div className="relative group">
            <Link to="#" className="flex items-center gap-1 cursor-pointer hover:text-gold transition-colors">
          ...
          <Link to={megaMenuConfig.main.find(item => item.label === "Store")?.href || "#"} className="cursor-pointer hover:text-gold transition-colors">
            {megaMenuConfig.main.find(item => item.label === "Store")?.label}
          </Link>
          {/* 3. About Dropdown - Already Hover-based */}
          <div className="relative group">
            <button
      ...
      {mobileMenuOpen && (
        <div className="md:hidden absolute top-16 left-0 w-full bg-indigo-900 border-t border-gold/20 text-white px-4 pb-6 pt-4 space-y-4">
          ...
          <Link to={megaMenuConfig.main.find(item => item.label === "Store")?.href || "#"} className="block text-sm text-white/90 hover:text-gold transition" onClick={() => setMobileMenuOpen(false)}>
            {megaMenuConfig.main.find(item => item.label === "Store")?.label}
          </Link>
          <div className="space-y-2">
            <h3 className="text-xs uppercase tracking-wider text-white/50 mt-4 mb-1 select-none">
