...
interface ImageSwiperProps {
  ...
  cardHeight?: number;
  className?: string;
  onSwipe?: (newIndex: number) => void; // Added the missing onSwipe prop
}
...
export const ImageSwiper: React.FC<ImageSwiperProps> = ({
  ...
  className = '',
  onSwipe // Added onSwipe to destructuring
}) => {
  ...
  const handleEnd = useCallback(() => {
      ...
      if (Math.abs(deltaX) > threshold) {
        ...
        setTimeout(() => {
          setCardOrder(prev => {
            ...
            // Calculate the new active card index and call onSwipe
            if (onSwipe) {
              const newActiveIndex = newOrder[0];
              onSwipe(newActiveIndex);
            }
    ...
    startX.current = 0;
    currentX.current = 0;
  }, [getDurationFromCSS, getActiveCard, applySwipeStyles, onSwipe]); // Added onSwipe to dependencies
  ...
  // Add effect to call onSwipe when component mounts with initial active card
  useEffect(() => {
    if (onSwipe && cardOrder.length > 0) {
      onSwipe(cardOrder[0]);
    }
  }, []); // Only run on mount
