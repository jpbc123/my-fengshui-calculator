...
useEffect(() => {
  const fetchDailyMessage = async () => {
    ...
    try {
      // Updated fetch call to use the relative path
  ...
  return (
    <div className="container mx-auto px-4 py-8 grid grid-cols-1 md:grid-cols-4 gap-8 items-center">
      <section className="md:col-span-2 py-8 px-4 bg-gradient-to-r from-purple-800 to-indigo-800 text-white rounded-lg shadow-xl transform transition-all duration-300 hover:scale-[1.01] hover:shadow-2xl cursor-pointer">
        <Link to={articlePath} onClick={storeFullArticle} className="block w-full h-full no-underline">
          <div className="flex flex-col items-center md:flex-row md:items-start justify-between space-y-4 md:space-y-0 md:space-x-8">
            ...
            <div className="flex flex-col text-center md:text-left flex-grow">
              <h2 className="text-xl md:text-2xl font-bold mb-2">Today's Insight</h2>
              <p className="text-lg md:text-l font-medium italic">
                "{partialMessage}"
              </p>
              <span className="mt-4 md:mt-6 inline-flex justify-center md:justify-start text-gold hover:underline">
                Read More →
              </span>
            </div>
          </div>
      ...
      <div className="hidden md:block border border-gray-300 rounded-lg p-4 h-full flex items-center justify-center bg-gray-100 text-gray-500">
        <p className="text-center">Ad Placeholder</p>
      </div>
    </div>
  );
};
